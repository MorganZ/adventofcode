...
const markedBoards = lines.map(() => Array(25).fill(0));
...
for (let bingoNumber of bingoNumbers)
    for (let bi = 0; bi < boards.length; bi++) {
        ...
        const b = boards[bi], mb = markedBoards[bi];
        let foundIndex = b.findIndex((v) => v == bingoNumber);
        if (foundIndex == -1) continue;
        mb[foundIndex] = 1;
        const ri = (foundIndex - foundIndex % 5);//row index
        const ci = foundIndex % 5; //col index
        if (mb[ri] + mb[ri + 1] + mb[ri + 2] + mb[ri + 3] + mb[ri + 4] == 5 || //row
            mb[ci] + mb[ci + 5 * 1] + mb[ci + 5 * 2] + mb[ci + 5 * 3] + mb[ci + 5 * 4] == 5) { //col
            boardsWin.add(bi);
