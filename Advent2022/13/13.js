const signals = require('fs').readFileSync('./13/input.txt', 'utf-8').split("\n\n").map(s => s.split("\n").map(p => JSON.parse(p)));
...
const result = signals.map((s, i) => [i, 0 < -1*compareSignal(s[0], s[1])])
    .filter(s => s[1]).reduce((p, c) => p + c[0] + 1, 0);
console.log(result);
